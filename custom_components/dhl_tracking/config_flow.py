...
async def _validate_credentials(
    ...
    """Validierung per HTTP Basic Auth."""
    try:
        async with aiohttp.ClientSession() as session:
            if api_type == API_TYPE_PARCEL_DE:
                ...
                url = (
                    f"{UNIFIED_API_SANDBOX_URL if sandbox else UNIFIED_API_URL}"
                    "?trackingNumber=validationtest"
                )
                async with session.get(
                    url,
...
class DhlTrackingOptionsFlow(OptionsFlow):
    ...
    async def async_step_email_scanner(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        errors: dict[str, str] = {}
        ...
        if user_input is not None:
            ...
            # Server aus Provider-Preset ermitteln
