...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # IMAP-Scanner starten wenn aktiviert
...
async def _async_start_imap_scanner(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Startet den IMAP-Scanner wenn er in den Optionen aktiviert ist."""
...
async def _async_stop_imap_scanner(hass: HomeAssistant, entry_id: str) -> None:
    """Stoppt einen laufenden IMAP-Scanner."""
    scanners = hass.data.get(IMAP_SCANNER_KEY, {})
    scanner = scanners.pop(entry_id, None)
    if scanner:
